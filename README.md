...
jobs:
  claude-response:
    ...
    steps:
      - uses: acertak/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: acertak/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`acertak/claude-code-base-action`](https://github.com/acertak/claude-code-base-action).
